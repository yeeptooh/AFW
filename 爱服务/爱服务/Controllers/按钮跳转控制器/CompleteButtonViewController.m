...
- (void)viewDidLoad {
    ...
    if ([self.inOut isEqualToString:@"保内"]) {
        ...
        self.tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, Width, (Height - StatusBarAndNavigationBarHeight)*11/12) style:UITableViewStylePlain];
    }
...
- (void)submitClicked:(UIButton *)sender {
    ...
    if ([self.inOut isEqualToString:@"保外"]) {
        ...
        NSDictionary *params = @{
                                 ...
                                 @"price":((UITextField *)[self.tableView viewWithTag:1005]).text,
                                 @"randNum":((UITextField *)[self.tableView viewWithTag:1006]).text
